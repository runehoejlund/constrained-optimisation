...
% Himmelblau's Problem - Solved with SQP + damped BFGS implementation.
clear
close all
cd(fileparts(which('Problem_4_6.m')))
...
x2c1 = @(x1) (x1+2).^2;
x2c2 = @(x1) 4/10*x1;
fill([xl x1 xu],[1.5*xu min(x2c1(x1),xu) 1.5*xu],[0.7 0.7 0.7],'LineWidth',1,'facealpha',0.2)
...
for i=1:size(x0s,2)
    ...
    B0 = diag(abs(eig(B)));
    [x, lambda, f, k] = SQPLocalSolver(@Himmelblau, @constraintsHimmelblau, x0, lambda0, B0);
